...
//        NSArray *dataSource = [NSArray arrayWithContentsOfFile:filePath];
        ...
        //自用
        NSBundle *bundle = [NSBundle bundleForClass:[self class]];
        NSURL *url = [bundle URLForResource:@"AMAddressSource" withExtension:@"bundle"];
        NSBundle *plistBundle = [NSBundle bundleWithURL:url];

        NSString *filePath = [plistBundle pathForResource:@"hhdAddress" ofType:@"plist"];
        NSArray *dataSource = [NSArray arrayWithContentsOfFile:filePath];
...
- (void)parseDataSource {
    ...
    for (NSDictionary *proviceDic in self.dataSource) {
        ...
        NSArray *citylist = proviceDic[@"childList"];
        ...
        for (NSDictionary *cityDic in citylist) {
            ...
            NSArray *arealist = cityDic[@"childList"];
